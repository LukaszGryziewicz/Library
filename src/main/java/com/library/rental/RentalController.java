...
public class RentalController {
    ...
    public ResponseEntity<List<Rental>> getRentalsOfCustomer(@PathVariable("id") Long id) {
        final List<Rental> rentals = rentalService.getRentalsOfCustomer(id);
        return new ResponseEntity<>(rentals, HttpStatus.OK);
    }

    @GetMapping("/bookRentals/{id}")
    public ResponseEntity<List<Rental>> getRentalsOfBook(@PathVariable("id") Long id) {
